...
export const createPost = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!imageUrl) return new Error("Invalid storage id");
    //create post
...
export const getFeedPosts = query({
  handler: async (ctx) => {
    ...
    //enhance posts with user data and interaction stuff (likes, comments)
...
export const toggleLike = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      ...
      // if this is not current user post then send a notification
